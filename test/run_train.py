...
def model_dict_load(model_dict, config_path, config_name, verbose=True):
    ...
    if model_dict is None:
        ...
        model_dict_fun = load_function_uri(
...
def train(model_dict, dfX, cols_family, post_process_fun):
    ...
    if ypred_proba is None:
        ...
        ypred_proba_val = ypred_proba[ival:, :]
        # merge into string "p1,p2,p3,p4"
        dfX[coly + '_proba'] = np_conv_to_one_col(ypred_proba, ";")
        log(dfX.head(3).T)
...
def run_train(config_name, config_path="source/config_model.py", n_sample=5000,
    ...
    :return:
    """
    model_dict = model_dict_load(
        model_dict, config_path, config_name, verbose=True)

    m = model_dict['global_pars']
    path_data_train = m['path_data_train']
    path_train_X = m.get('path_train_X', path_data_train +
                         "/features.zip")  # .zip
    path_train_y = m.get(
        'path_train_y', path_data_train + "/target.zip")  # .zip
    ...
    model_dict['data_pars']['cols_model'] = sum(
        [cols[colgroup] for colgroup in model_dict['data_pars']['cols_model_group']], [])
    ...
    if return_mode == 'dict':
        ...
        dfXy[colexport].reset_index().to_csv(
            path_check_out + "/pred_check.csv")  # Only results
        # train input data generate parquet
        dfXy.to_parquet(path_check_out + "/dfX.parquet")
        # dfXy.to_csv(path_check_out + "/dfX.csv")  # train input data generate csv
        # Test input data  generate parquet
        dfXytest.to_parquet(path_check_out + "/dfXtest.parquet")
