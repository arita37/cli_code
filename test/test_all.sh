...
python   cli_code/cli_download -u https://drive.google.com/file/d/1FPn4Q4PClobHgEU4DglyF2Xbs5Boe1r_/view?usp=sharing -o test_out/


# this one requires git and conda installed on runner
python   cli_code/cli_repo_check https://github.com/parul1931/walmart.git -n testing


# check cli_conda_merge by joining two test conda env files
python   cli_code/cli_conda_merge test/file0.yaml test/file1.yaml


# check cli_module_parser by parsing .py files in test dir
python   cli_code/cli_module_parser test/ -o test_out/module_parsed.csv


# check cli_env_autoinstall by creating an environemnt for test repo
python   cli_code/cli_env_autoinstall test/ -n notebook_cvt


# check cli_github_search by searching for some keywords
python   cli_code/cli_github_search amazon scraper -o test_out/
